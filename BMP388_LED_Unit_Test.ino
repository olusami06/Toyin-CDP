...
const int greenPin = 4; // Define pin on Teensy for Green
const int bluePin = 3; // Define pin on Teensy for Blue
...
void setup() {
  ...
  if (!SD.begin(BUILTIN_SDCARD)) {  // Teensy 4.1 has a built-in SD card reader
    ...
    digitalWrite(LED_BUILTIN, HIGH);  // Turn on red LED for SD card failure
...
void loop() {
  ...
  if (millis() - startTime >= 60000) { // 1 minute in milliseconds
    ...
    digitalWrite(bluePin, LOW);   // Turn on blue LED
